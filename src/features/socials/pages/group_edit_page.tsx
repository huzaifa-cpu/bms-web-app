...
export default function GroupEditPage() {
  ...
  useEffect(() => {
    if (group) {
      ...
      setAvatarPreview(group.imageUrl)
      // Load group members
      if (group.members && group.members.length > 0) {
        const loadedMembers = group.members.map(m => ({
          id: m.userId,
  ...
  return (
    <div>
      <h4 className="fw-bold mb-4">Edit Group — {group.name}</h4>
      <Card className="shadow-sm" style={{ maxWidth: 600 }}>
        <Card.Body>
          <div className="text-center mb-4">
            ...
            >
              {avatarPreview ? (
                ...
                <div className="rounded-circle bg-secondary d-flex align-items-center justify-content-center" style={{ width: 80, height: 80 }}>
                  <span className="text-white fs-3">{group.name[0].toUpperCase()}</span>
                </div>
              )}
            ...
            <p className="text-muted small mt-2 mb-0">Click to change group image</p>
          </div>
