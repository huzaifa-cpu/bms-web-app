...
type Member = { id: number; name: string; phone: string; isOwner: boolean }
...
export default function GroupCreatePage() {
  ...
  const addMember = (user: { id: number; name: string; mobileNumber: string }) => {
    if (members.length >= 100) {
      toast.error('Maximum 100 members allowed')
      return
    }
    setMembers(prev => [...prev, { id: user.id, name: user.name, phone: user.mobileNumber, isOwner: prev.length === 0 }])
    setMemberSearch('')
  ...
  const removeMember = (userId: number) => {
    setMembers(prev => {
      ...
      // If removed member was owner and there are still members, make first one owner
      if (prev.find(m => m.id === userId)?.isOwner && updated.length > 0) {
    ...
    try {
      await createGroup({
        request: {
          ...
          members: members.map(m => ({ userId: m.id, admin: m.isOwner })),
        },
        image: avatarFile ?? undefined,
      }).unwrap()
      toast.success('Group created successfully.')
      navigate('/socials/groups')
    } catch {
  ...
  return (
    <div>
      <h4 className="fw-bold mb-4">Create Group</h4>
      <Card className="shadow-sm" style={{ maxWidth: 600 }}>
        <Card.Body>
          <div className="text-center mb-4">
            ...
            >
              {avatarPreview ? (
                <img src={avatarPreview} alt="Group Avatar" className="rounded-circle" style={{ width: 80, height: 80, objectFit: 'cover' }} />
              ) : (
            ...
            <p className="text-muted small mt-2 mb-0">Click to add group image</p>
          </div>
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Form.Group className="mb-3">
              <Form.Label>Group Name <span className="text-danger">*</span></Form.Label>
              <Form.Control isInvalid={!!errors.name} {...register('name')} placeholder="e.g., Cricket Lovers Lahore" />
            ...
            {/* Group Members */}
            <Form.Group className="mb-3">
              <Form.Label>Group Members <span className="text-danger">*</span> <small className="text-muted">(1-100 members)</small></Form.Label>
              ...
              {members.length > 0 && (
                <div className="mt-3">
                  <small className="text-muted d-block mb-2">Added Members ({members.length}/100):</small>
                  <ListGroup>
                    {members.map(m => (
                      <ListGroup.Item key={m.id} className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="d-flex align-items-center gap-2">
                          {m.isOwner && <Badge bg="warning" text="dark"><BsShieldFill className="me-1" />Admin</Badge>}
                          <div className="d-flex gap-1">
                            {!m.isOwner && (
                              <Button size="sm" variant="outline-warning" title="Make Admin" onClick={() => setOwner(m.id)}>
                                <BsShieldFill />
                              </Button>
                            )}
            ...
            <div className="d-flex gap-2">
              <Button type="submit" variant="primary" disabled={isCreating}>{isCreating ? 'Creating...' : 'Create Group'}</Button>
              <Button variant="secondary" onClick={() => navigate('/socials/groups')}>Cancel</Button>
            </div>
          </Form>
        </Card.Body>
      </Card>
